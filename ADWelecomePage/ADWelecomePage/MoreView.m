...
- (void)setWeatherModel:(WeatherModel *)weatherModel{
    ...
    WeatherDetailModel *detailModel = [WeatherDetailModel objectWithKeyValues:weatherModel.detailArray[0]] ;
    PM2d5Model *pmModel = weatherModel.pm2d5;
    
    self.nowTempure.text = [NSString stringWithFormat:@"%d",weatherModel.rt_temperature];
    self.timeLable.text = [NSString stringWithFormat:@"%@  %@",weatherModel.dt,detailModel.week];
    self.airQuality.text = [NSString stringWithFormat:@"PM2.5 %@ 差",pmModel.pm2_5];
    self.weatherLabel.text = [NSString stringWithFormat:@"%@ %@",detailModel.climate,detailModel.wind];
}
...
+ (instancetype)view{
    return [[NSBundle mainBundle]loadNibNamed:@"MoreView" owner:nil options:nil].firstObject;
}
